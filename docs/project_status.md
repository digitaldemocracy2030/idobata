...
> 注意: 本ドキュメントのマイルストーン（第 3 節）は 2025/4/29 時点の計画で、現在の状況とは異なる場合があります。最新情報は Slack や Github Project を確認してください。なお「現在のシステム構成」（第 2 節）は、`vision/` `policy/` への再編および policy バックエンドの MCP 廃止（GitHub 通信のバックエンド内直接実行）を反映した最新の構成です。
...
- **いどばた政策**の各コンポーネントは`policy`ディレクトリ内に配置されています
...
```
.
├── vision   # いどばたビジョン＋いどばた政策の管理画面・ユーザー画面
│             ├── admin           （統合された管理画面）
│             ├── frontend        （統合されたユーザー画面）
│             ├── idea-discussion
