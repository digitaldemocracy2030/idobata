...
const getThreadByUserAndTheme = async (req, res) => {
  ...
  try {
    ...
    if (!chatThread) {
      ...
      console.log(
        `Processing automatic greeting for new thread ${newChatThread._id}`
      );
      const greetingProcessed = await processAutomaticMessage(
        newChatThread,
        "（スレッド開始）",
        themeId
      );
      ...
      if (!greetingProcessed) {
        console.warn(
          `Failed to process automatic greeting for thread ${newChatThread._id}`
        );
      }
      ...
      return res.status(200).json({
        ...
        userId: userId,
        themeId: themeId,
        messages: newChatThread.messages || [],
      });
    }
...
const processAutomaticMessage = async (chatThread, message, themeId) => {
  try {
    chatThread.messages.push({
      role: "user",
      content: message,
...
`;
    ...
    if (!aiResponseContent) {
      console.error("LLM did not return a response for automatic greeting.");
      return false;
    }
    ...
    await chatThread.save();
    console.log(
      `Saved automatic greeting for thread ${chatThread._id} in theme: ${themeId}`
    );
    ...
    setTimeout(() => {
      ...
      processExtraction(job).catch((err) => {
        console.error(
          `[Async Extraction Call] Error for automatic greeting thread ${chatThread._id} in theme ${themeId}:`,
          err
        );
      });
    }, 0);
    ...
    console.error(
      `Error processing automatic greeting for theme ${themeId}:`,
      error
    );
    return false;
  }
